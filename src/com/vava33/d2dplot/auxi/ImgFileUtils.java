...
public final class ImgFileUtils {
    ...
    public static FileNameExtensionFilter[] getExtensionFilterWrite() {
        ...
        final Iterator<String> itrformats = ImgFileUtils.formatInfo.keySet().iterator();
        final FileNameExtensionFilter[] filter = new FileNameExtensionFilter[ImgFileUtils.SupportedWriteExtensions
                .values().length];
        int nfiltre = 0;
        while (itrformats.hasNext()) {
            final String frm = itrformats.next();
            // this line returns the FORMAT in the ENUM or NULL
            final ImgFileUtils.SupportedWriteExtensions wfrm = FileUtils
                    .searchEnum(ImgFileUtils.SupportedWriteExtensions.class, frm);
            if (wfrm != null) {
                // afegim filtre
    ...
    public static FileNameExtensionFilter[] getExtensionFilterRead() {
        // mirem quins formats som capaços de salvar segons ImgFileUtils
        final Iterator<String> itrformats = ImgFileUtils.formatInfo.keySet().iterator();
        final FileNameExtensionFilter[] filter = new FileNameExtensionFilter[ImgFileUtils.SupportedReadExtensions
                .values().length + 1]; // +1 for all image formats
        final String[] frmStrings = new String[ImgFileUtils.SupportedReadExtensions.values().length];
        int nfiltre = 1; //comença a 1 perque el zero el reservem per tots els formats
        while (itrformats.hasNext()) {
            final String frm = itrformats.next();
            // this line returns the FORMAT in the ENUM or NULL
            final ImgFileUtils.SupportedReadExtensions wfrm = FileUtils
                    .searchEnum(ImgFileUtils.SupportedReadExtensions.class, frm);
            if (wfrm != null) {
                // afegim filtre
                filter[nfiltre] = new FileNameExtensionFilter(ImgFileUtils.formatInfo.get(frm), frm);
        ...
        filter[0] = new FileNameExtensionFilter("All 2D-XRD supported formats", frmStrings);
        return filter;
    }

    //returns the new value if line matches, otherwise the previous one
    private static int getIntKeyword(String line, String keyword, String separator, int corrLastCharacter,
            int currentValue) {
        ...
        if (FileUtils.containsIgnoreCase(line, keyword)) {
            final int iigual = line.trim().indexOf(separator) + 1;
    ...
    //returns the new value if line matches, otherwise the previous one
    private static float getFloatKeyword(String line, String keyword, String separator, int corrLastCharacter,
            float currentValue) {
        //retorna un enter despres d'una keyword donada i el simbol de separacio, treu l'ultim caracter si corrLastCharacter es 1 (aplicarà -1)
        if (FileUtils.containsIgnoreCase(line, keyword)) {
            final int iigual = line.trim().indexOf(separator) + 1;
    ...
    private static String getStringKeyword(String line, String keyword, String separator, int corrLastCharacter,
            String currentValue) {
        //retorna un enter despres d'una keyword donada i el simbol de separacio, treu l'ultim caracter si corrLastCharacter es 1 (aplicarà -1)
        if (FileUtils.containsIgnoreCase(line, keyword)) {
            final int iigual = line.trim().indexOf(separator) + 1;
    ...
    public static Pattern2D readPatternFile(File d2File, boolean exzConfirm) {
        ...
        final String ext = FileUtils.getExtension(d2File).trim();
        ...
        if (format == null) {
            final SupportedReadExtensions[] possibilities = SupportedReadExtensions.values();
            final SupportedReadExtensions s = (SupportedReadExtensions) JOptionPane.showInputDialog(null,
                    "Input format:", "Read File", JOptionPane.PLAIN_MESSAGE, null, possibilities, possibilities[0]);
            if (s == null) {
                return null;
            }
            format = s;
    ...
    public static File writePatternFile(File d2File, Pattern2D patt2D, boolean forceExt) {
        ...
        if (format == null) {
            ...
            final SupportedWriteExtensions s = (SupportedWriteExtensions) JOptionPane.showInputDialog(null,
                    "Output format:", "Save File", JOptionPane.PLAIN_MESSAGE, null, possibilities, possibilities[0]);
            if (s == null) {
                return null;
            }
            format = s;
    ...
    private static Pattern2D readBIN(File d2File) {
        Pattern2D patt2D = null;
        try {
            final long start = System.nanoTime(); // control temps
            final InputStream in = new BufferedInputStream(new FileInputStream(d2File));
            final byte[] buff4 = new byte[4]; // real
            float scale = -1;
            float centrX, centrY, pixlx, pixly, sepod, wl, omeIni, omeFin, acqTime;
            int dimX, dimY;
            final int totalHeaderBytes = 60;
    ...
    private static boolean isNewBIN(File d2File) {
        ...
        // NX*NY*2 + 20 = old
        // NX*NY*2 + 60 = new
        final int limit = dimX * dimY * 2 + 20;
        InputStream stream = null;
        int bytes = 0;
        try {
            final URL url = d2File.toURI().toURL();
        ...
        try {
            ...
            for (int j = 0; j < patt2D.getDimY(); j++) { // per cada fila (Y)
                for (int i = 0; i < patt2D.getDimX(); i++) { // per cada columna (X)
                    in.read(buff);
                    patt2D.setInten(i, j, (FileUtils.ByteArrayToInt_signed(buff, true)), true);//estava malament, era B2toInt_LE_unsigned(buff))
                    count = count + 1;
                    // nomes considerem valors superiors a zero pel minI
                    if (patt2D.getInten(i, j) >= 0) {
                        if (patt2D.getInten(i, j) > patt2D.getMaxI())
                            patt2D.setMaxI(patt2D.getInten(i, j));
                        if (patt2D.getInten(i, j) < patt2D.getMinI())
                            patt2D.setMinI(patt2D.getInten(i, j));
                    }
                }
            }
            in.close();
    ...
    private static Pattern2D readGFRM(File d2File) {
        ...
        try {
            ...
            final Scanner scD2file = new Scanner(new BufferedReader(new FileReader(d2File)));
            final String line = scD2file.nextLine();
            log.debug(line);
            // treurem la informaci� d'aquesta linia.
            // 0 1 2 3 4 5 6 7 8
            final String[] llista = { "HDRBLKS:", "NROWS  :", "NCOLS  :", "CENTER :", "DISTANC:", "NOVERFL:",
                    "MINIMUM:", "MAXIMUM:", "NPIXELB:" };
            for (int i = 0; i < llista.length; i++) {
                final Scanner scLine = new Scanner(line);
                ...
                if (scLine.hasNext()) {
                    ...
                    final StringBuffer item = new StringBuffer(72);
                    for (int j = 0; j < 71; j++) {
                    ...
                    if (i == 3) {
                        final String[] s = item.toString().trim().split("\\s+");
            ...
            final InputStream in = new BufferedInputStream(new FileInputStream(d2File));
            final byte[] buff = new byte[nbytePerPixel];
            final byte[] header = new byte[headerSize * 512];
            ...
            for (int j = 0; j < patt2D.getDimY(); j++) { // per cada fila (Y)
                for (int i = 0; i < patt2D.getDimX(); i++) { // per cada columna (X)
                    in.read(buff);
                    final ByteBuffer buffer = ByteBuffer.wrap(buff);
                    ...
                    patt2D.setInten(i, j, valorLlegit, true);
                    count = count + 1;
            ...
            if (noverflow > 0) {
                for (int i = 0; i < noverflow; i++) {
                    final byte[] buffchar = new byte[16];
                    in.read(buffchar);

                    StringBuffer sb = new StringBuffer(10);
                    for (int j = 0; j < 9; j++) {
                        final int c = FileUtils.B1toInt_unsigned(buffchar[j]);
                        sb.append((char) c);
                    }
                    final int intensity = Integer.parseInt(sb.toString().trim());
                    ...
                    final int offset = Integer.parseInt(sb.toString().trim());

                    // ara calculem quin pixel es
                    final int fila = offset / dimX; // LA Y
                    final int col = (offset % dimX); // +1 //LA X

                    patt2D.setInten(col, fila, intensity, true);
                }
            }
            in.close();
            final long end = System.nanoTime();
    ...
    private static Pattern2D readIMG(File d2File) {
        ...
        int dimX = 0, dimY = 0;
        final int maxI = 0, minI = 9999999;

        final int maxLinesHeader = 200;
        final int maxLineLength = 1000;
        ...
        try {
            ...
            for (int j = 0; j < patt2D.getDimY(); j++) { // per cada fila (Y)
                for (int i = 0; i < patt2D.getDimX(); i++) { // per cada columna (X)
                    in.read(buff);
                    final int valorLlegit = FileUtils.B2toInt_LE_unsigned(buff);
                    count = count + 1;
                    patt2D.setInten(i, j, FileUtils.B2toInt_LE_unsigned(buff), true);
    ...
    public static EdfHeaderPatt2D readEDFheaderOnly(File d2File) {
            ...
            while (!headerFinished) {
                if (scD2file.hasNextLine()) {
                    ...
                    if (FileUtils.containsIgnoreCase(line, "ByteOrder =")) {
                        final String endiness = line.substring(iigual, line.trim().length() - 1).trim();
                    ...
                    try {
                        ...
                        if (FileUtils.containsIgnoreCase(line, "scan_type")) {
                            final String line2 = line.substring(iigual, line.trim().length() - 1).trim();
                            if (FileUtils.containsIgnoreCase(line2, "mar_scan")) {
                                final String[] values = line2.split(",");
        ...
        final Pattern2D patt2D = new Pattern2D(dimX, dimY, beamCX, beamCY, maxI, minI, -1.0f);
        patt2D.setExpParam(pixSizeX, pixSizeY, distOD, wl, tilt, rot);
        patt2D.setScanParameters(omeIni, omeFin, acqTime);

        final int headerSize = (int) (d2File.length() - binSize);
        log.config("EDF header size (bytes)=" + headerSize);
        log.writeNameNumPairs("CONFIG", true, "dimX,dimY,beamCX,beamCY,pixSizeX,distOD,wl", dimX, dimY, beamCX, beamCY,
                pixSizeX, distOD, wl);
    ...
    private static Pattern2D readEDF(File d2File) {
        final long start = System.nanoTime(); // control temps
        final EdfHeaderPatt2D edfinf = readEDFheaderOnly(d2File);
        final Pattern2D patt2D = edfinf.getPatt2D();
        patt2D.setSaturValue(D2Dplot_global.satur65);

        // primer treiem la info de les linies de text
        try {
            if (edfinf.getHeaderSize() < 0)
                throw new Exception();
            ...
            if (edfinf.getDataType().contains("SignedInt")) {
                ...
                final DataInputStream in = new DataInputStream(new FileInputStream(d2File));
                final byte[] buff = new byte[4];
                final byte[] header = new byte[edfinf.getHeaderSize()];
                //                patt2D = new Pattern2D(dimX, dimY, beamCX, beamCY, maxI, minI,-1.0f);
                int count = 0;
                in.read(header);
                ...
                for (int j = 0; j < patt2D.getDimY(); j++) { // per cada fila (Y)
                    for (int i = 0; i < patt2D.getDimX(); i++) { // per cada columna (X)
                        in.read(buff);
                        final int valorLlegit = FileUtils.B4toInt_LE_signed(buff);
                        count = count + 1;
                        patt2D.setInten(i, j, valorLlegit, true);
                        if (valorLlegit >= 0) { // fem >= o > directament sense considerar els zeros??!
                            if (valorLlegit > patt2D.getMaxI())
    ...
    private static EdfHeaderPatt2D readD2Dheader(File d2File) {
        ...
        final ArrayList<PolyExZone> polyExzones = new ArrayList<PolyExZone>();
        final ArrayList<ArcExZone> arcExzones = new ArrayList<ArcExZone>();
        final ArrayList<Point> exzPoints = new ArrayList<Point>();
        float omeIni = 0, omeFin = 0, acqTime = -1;
        ...
        try {
            ...
            while (!headerFinished) {
                ...
                if (scD2file.hasNextLine()) {
                    ...
                    if (FileUtils.containsIgnoreCase(line, "EXZpol")) {
                        final String linia = line.substring(iigual, line.trim().length()).trim();
                        final String[] values = linia.split("\\s+");
                        final PolyExZone z = new PolyExZone(false);
                    ...
                    if (FileUtils.containsIgnoreCase(line, "EXZarc")) {
                        final String linia = line.substring(iigual, line.trim().length()).trim();
                        final String[] values = linia.split("\\s+");
                        final int ipx = Integer.parseInt(values[0]);
                        final int ipy = Integer.parseInt(values[1]);
                        final int hradwpx = Integer.parseInt(values[2]);
                        final int hazimwdeg = Integer.parseInt(values[3]);
                    ...
                    if (FileUtils.containsIgnoreCase(line, "EXZpix")) {
                        final String linia = line.substring(iigual, line.trim().length()).trim();
                        final String[] values = linia.trim().split("\\s+");
                        log.debug(linia);
                        ...
                        if (values.length > 1) {
                            for (int j = 0; j < values.length; j = j + 2) {
                                try {
                                    exzPoints.add(
                                            new Point(Integer.parseInt(values[j]), Integer.parseInt(values[j + 1])));
                                } catch (final Exception e) {
            ...
            log.writeNameNumPairs("CONFIG", true, "dimX,dimY,beamCX,beamCY,pixSizeX,distOD,wl,tilt,rot", dimX, dimY,
                    beamCX, beamCY, pixSizeX, distOD, wl, tilt, rot);
    ...
    private static Pattern2D readD2D(File d2File) {
        ...
        final EdfHeaderPatt2D edfinf = readD2Dheader(d2File);
        final Pattern2D patt2D = edfinf.getPatt2D();
        patt2D.setSaturValue(D2Dplot_global.satur65);
    ...
    private static Pattern2D readTIFF(File d2File) {
        ...
        final ImagePlus imp = op.openImage(d2File.getAbsolutePath());
        final ImageProcessor sp = imp.getProcessor();

        Pattern2D patt2D = null;
        final long start = System.nanoTime(); // control temps
        float pixSizeX = 0, pixSizeY = 0;
        final float distOD = 0;
        final float beamCX = 0, beamCY = 0, wl = 0;
        int dimX = 0, dimY = 0;
        final int maxI = 0, minI = 9999999;
        ...
        final String xunit = cal.getXUnit();
        float factor = 1.0f;
            ...
            for (int i = 0; i < 80; i++) {
                if (scD2file.hasNextLine()) {
                    ...
                    if (FileUtils.containsIgnoreCase(line, "Beam_xy")) {
                        ...
                        try {
                            final String s1 = line.trim().split(Pattern.quote("("))[1]; //Beam_xy (1237.95, 1302.57) pixels
                            final String s2 = s1.trim().split(Pattern.quote(")"))[0];
                            final String[] vals = s2.trim().split(",");
                    ...
                    if (FileUtils.containsIgnoreCase(line, "Pixel_size")) {
                        final String[] vals = line.trim().split("\\s+");
                        try {
                    ...
                    if (FileUtils.containsIgnoreCase(line, "Angle_increment")) {
                        final String[] vals = line.trim().split("\\s+");
                        try {
                            final float incr = Float.parseFloat(vals[2]);
                            omeFin = omeIni + incr;
                        } catch (final Exception ex) {
     ...
     * 
     * If -127 ≤ delta ≤ 127, add delta to the base pixel value, make that the new base pixel value, place it on the
     ...
     * 
     * If -32767 ≤ delta ≤ 32767, add delta to the base pixel value, make that the new base pixel value, place it on the
     * output array and return to step 2.
     * 
     * If delta is 8000 hex, read the next 4 bytes as a little_endian 32-bit number and make that delta
     * 
     * If -2147483647 ≤ delta ≤ 2147483647, add delta to the base pixel value, make that the new base pixel value, place
     * it on the output array and return to step 2.
     * 
     * If delta is 80000000 hex, read the next 8 bytes as a little_endian 64-bit number and make that delta, add delta
     * to the base pixel value, make that the new base pixel value, place it on the output array and return to step 2.
     */
    ...
    private static Pattern2D readCBF(File d2File) {
        ...
        try {
            ...
            for (int i = 0; i < 80; i++) {
                if (scD2file.hasNextLine()) {
                    ...
                    content_transfer_encoding = getStringKeyword(line, "Content-Transfer-Encoding", ":", 0,
                            content_transfer_encoding);
                    binary_Element_Type = getStringKeyword(line, "Binary-Element-Type", ":", 0, binary_Element_Type);
                    binary_element_byte_order = getStringKeyword(line, "Binary-Element-Byte-Order", ":", 0,
                            binary_element_byte_order);
                    ...
                    binary_size_fastest_dim = getIntKeyword(line, "Binary-Size-Fastest-Dimension", ":", 0,
                            binary_size_fastest_dim);
                    binary_size_second_dim = getIntKeyword(line, "Binary-Size-Second-Dimension", ":", 0,
                            binary_size_second_dim);
        ...
        try {
            ...
            int basePixel = 0;
            final byte[] buff2 = new byte[2];
            final byte[] buff4 = new byte[4];
            final byte[] buff8 = new byte[8];
            ...
            for (int j = 0; j < patt2D.getDimY(); j++) { // per cada fila (Y)
                for (int i = 0; i < patt2D.getDimX(); i++) { // per cada columna (X)
                    ...
                    basePixel = intensity;
                    patt2D.setInten(i, j, intensity, true);
                    count = count + 1;
                }
            }
            in.close();
        ...
        log.writeNameNumPairs("CONFIG", true, "dimX,dimY,beamCX,beamCY,pixSizeX,distOD,wl", patt2D.getDimX(),
                patt2D.getDimY(), patt2D.getCentrX(), patt2D.getCentrY(), patt2D.getPixSx(), patt2D.getDistMD(),
                patt2D.getWavel());
    ...
    private static Pattern2D readSPR(File d2File) {
        ...
        try {
            ...
            for (int j = 0; j < patt2D.getDimY(); j++) { // per cada fila (Y)
                for (int i = 0; i < patt2D.getDimX(); i++) { // per cada columna (X)
                    final String r = scD2file.next();
                    count = count + 1;
                    final int in = new java.math.BigDecimal(r).intValue();
                    patt2D.setInten(i, j, in, true);
    ...
    private static File writeBIN(File d2File, Pattern2D patt2D, boolean forceExt) {
        ...
        final int dataHeaderBytes = 48; // bytes de dades en el header
    ...
    private static File writeEDF(File d2File, Pattern2D patt2D, boolean forceExt) {
        ...
        final int binSize = patt2D.getDimX() * patt2D.getDimY() * 2; // considerant 2 bytes per pixel
        ...
        try {
            ...
            // scan_type = mar_scan ('hp_som', -5.0, 5.0, 2.0) ;
            // scan_type = mar_ct (1.0,) ;
            final float omeIni = patt2D.getOmeIni();
            final float omeFin = patt2D.getOmeFin();
            final float acqTime = patt2D.getAcqTime();
            ...
            if ((FastMath.round(omeIni * 100) == 0) && (FastMath.round(omeFin * 100) == 0)) {
                ...
                // marScan
                output.println("scan_type = mar_scan ('hp_som', " + FileUtils.dfX_1.format(omeIni) + ", "
                        + FileUtils.dfX_1.format(omeFin) + ", " + FileUtils.dfX_1.format(acqTime) + ") ;");
            }
            output.println("}");
            output.close();

            // ara mirem quants bytes hem escrit...
            final int headerbytes = (int) d2File.length();
            log.config("Write EDF headerbytes=" + headerbytes);

            //mirem escala que hi càpiga en 65000
            final float sc65 = patt2D.calcScale(D2Dplot_global.satur65);

            final OutputStream os = new BufferedOutputStream(new FileOutputStream(d2File, true));
            ...
            for (int j = 0; j < patt2D.getDimY(); j++) { // per cada fila (Y)
                for (int i = 0; i < patt2D.getDimX(); i++) { // per cada columna (X)
                    ...
                    bb.order(ByteOrder.LITTLE_ENDIAN);
                    if (patt2D.getInten(i, j) > 0) {
                        bb.putChar((char) ((int) (patt2D.getInten(i, j) / sc65)));
                    } else {
                        bb.putChar((char) 0);
                    }
                    os.write(bb.array());
    ...
    private static File writeIMG(File d2File, Pattern2D patt2D, boolean forceExt) {
        ...
        try {
            ...
            // ara mirem quants bytes hem escrit...
            final int writtenHeaderBytes = (int) d2File.length();
    ...
    private static File writeD2D(File d2File, Pattern2D patt2D, boolean forceExt) {
        ...
        try {
            ...
            int polcount = 1;
            final Iterator<PolyExZone> it = patt2D.getPolyExZones().iterator();
            while (it.hasNext()) {
            ...
            int arccount = 1;
            final Iterator<ArcExZone> it2 = patt2D.getArcExZones().iterator();
            while (it2.hasNext()) {
                final ArcExZone p = it2.next();
                output.println(String.format("EXZarc%d=%d %d %d %.3f", arccount, p.getPx(), p.getPy(),
                        p.getHalfRadialWthPx(), p.getHalfAzimApertureDeg()));
                arccount = arccount + 1;
            }
            final Iterator<Point> it3 = patt2D.getExZpaintPixels().iterator();
            final StringBuilder sb = new StringBuilder();
            while (it3.hasNext()) {
                final Point p = it3.next();
    ...
    private static File writeTIFF(File d2File, Pattern2D patt2D, boolean forceExt) {
        ...
        final FileSaver fs = new FileSaver(ipl);
    ...
    private static File findEXZfile(File imgFile) {
        ...
        if (!trobat) {
            final File f = new File(D2Dplot_global.getWorkdir());
            log.debug(f.toString());
            final String prefix = imgFile.getName().substring(0, FastMath.min(4, imgFile.getName().length()));
            final File[] matchingFiles = f.listFiles(new filtreExz(prefix, "exz"));
    ...
    public static boolean readEXZ(Pattern2D patt2D, File exfile, boolean exzConfirm) {
        final File dataFile = patt2D.getImgfile();
        if (exfile == null) {
            ...
            if (exfile != null) {
                if (!exfile.exists())
                    return false;
                if (exzConfirm) {
                    final boolean readEXZ = FileUtils.YesNoDialog(null,
        ...
        String line;
        try {
            final Scanner scExFile = new Scanner(exfile);
            ...
            while (llegint) {
                ...
                if (FileUtils.containsIgnoreCase(line, "EXZthres")) {
                    final int yth = Integer.parseInt(line.substring(iigual, line.trim().length()).trim());
                    patt2D.setExz_threshold(yth);
                    continue;
                }
                if (FileUtils.containsIgnoreCase(line, "EXZdet")) {
                    final int ydc = Integer.parseInt(line.substring(iigual, line.trim().length()).trim());
                ...
                if (FileUtils.containsIgnoreCase(line, "EXZarc")) {
                    ...
                    final int ipy = Integer.parseInt(values[1]);
                    final int hradwpx = Integer.parseInt(values[2]);
                    final float hazimwdeg = Float.parseFloat(values[3]);
                    final ArcExZone a = new ArcExZone(ipx, ipy, hradwpx, hazimwdeg, patt2D);
                ...
                if (FileUtils.containsIgnoreCase(line, "EXZpix")) {
                    ...
                    final String[] values = linia.split("\\s+");
                    for (int i = 0; i < values.length; i = i + 2) {
                        patt2D.addPaintedEXZpixel(
                                new Point(Integer.parseInt(values[i]), Integer.parseInt(values[i + 1])));
    ...
    public static File writeEXZ(File exfile, Pattern2D patt2D, boolean forceExt) {
        ...
        try {
            final PrintWriter output = new PrintWriter(new BufferedWriter(new FileWriter(exfile)));
            // ESCRIBIM AL FITXER:
            ...
            while (it2.hasNext()) {
                final ArcExZone p = it2.next();
                output.println(String.format("EXZarc%d=%d %d %d %.1f", arccount, p.getPx(), p.getPy(),
                        p.getHalfRadialWthPx(), p.getHalfAzimApertureDeg()));
                arccount = arccount + 1;
            }
    ...
    public static File writeCALfile(File calfile, Pattern2D patt2D, com.vava33.d2dplot.Calibration c,
            boolean forceExt) {
        if (forceExt)
            calfile = new File(FileUtils.getFNameNoExt(calfile).concat(".cal"));
        final String tm = D2Dplot_global.getStringTimeStamp("yyyy-MM-dd 'at' HH:mm");

        try {
            final PrintWriter output = new PrintWriter(new BufferedWriter(new FileWriter(calfile)));
            ...
            output.println(String.format("OUTER RADIUS = %d", patt2D.getDimX() / 2 - 1));
            output.println(String.format("AZIMUTH BINS = %d", 1));
            output.println(String.format("RADIAL BINS = %d", patt2D.getDimX() / 2 - 1));
            output.println();
            output.println(String.format("#MASK FILE = %s",
                    D2Dplot_global.getWorkdir() + D2Dplot_global.separator + "MASK.bin"));
    ...
    public static boolean readCALfile(Pattern2D patt2D, File calfile, ConvertTo1DXRD ir) {
        String line;
        try {
            final Scanner scCALFile = new Scanner(calfile);
            ...
            if (ir != null) {
                ...
                ir.setTxtT2i(t2i);
                final float t2f = (float) patt2D.calc2T(FastMath.min(patt2D.getCentrX() + outer, patt2D.getDimX()),
                        patt2D.getCentrY(), true);
                ir.setTxtT2f(t2f);
                float step = (t2f - t2i) / radBins;
                if (step < patt2D.calcMinStepsizeBy2Theta4Directions())
    ...
    public static File readXDS(File xdsFile, Pattern2D patt2d) {
        ...
        try {
            ...
            final Scanner scSolfile = new Scanner(xdsFile);
            while (scSolfile.hasNextLine()) {
                final String line = scSolfile.nextLine();
                if (line.isEmpty())
                    continue;
                npunts = npunts + 1;
                log.debug("xdsfileline= " + line);
                final String lineS[] = line.trim().split("\\s+");
                patt2d.getSolucions().get(0).addSolPoint(npunts, Float.parseFloat(lineS[0]), Float.parseFloat(lineS[1]),
                        Integer.parseInt(lineS[4]), Integer.parseInt(lineS[5]), Integer.parseInt(lineS[6]), 1.0f,
    ...
    public static File readPCSasSOL(File pcsFile, Pattern2D patt2d) {
        ...
        String line;
        try {
            final Scanner scPCSfile = new Scanner(pcsFile);
            ...
            while (scPCSfile.hasNextLine()) {
                line = scPCSfile.nextLine();
                final String[] lineS = line.trim().split("\\s+");
                patt2d.getSolucions().get(0).addSolPoint(Float.parseFloat(lineS[1]), Float.parseFloat(lineS[2]), false);
                final int codi = Integer.parseInt(lineS[8]);
                boolean nearmask = false;
                int neixam = 1;
                int nsatur = 0;
                final Point2D.Float centre = new Point2D.Float(Float.parseFloat(lineS[1]), Float.parseFloat(lineS[2]));
                if (codi < -1) {
                    nsatur = codi * (-1);
                }
                if (codi == -1) {
    ...
    public static File writePCS(Pattern2D patt2d, File PCSfile, float delsig, float angDeg, boolean autoAngDeg,
            int zoneR, int minpix, int bkgpt, boolean autoBkgPt, boolean autoazim, boolean forceExt) {
        if (forceExt)
            PCSfile = new File(FileUtils.getFNameNoExt(PCSfile).concat(".PCS"));
        ...
        try {
            ...
            final PrintWriter output = new PrintWriter(new BufferedWriter(new FileWriter(PCSfile)));
            // ESCRIBIM AL FITXER:
            output.println(eqLine);
            output.println("D2Dplot (v" + D2Dplot_global.version + ") peak integration for TTS_INCO " + dt);
            output.println(eqLine);
    ...
    public static File readSOL(File solFile, Pattern2D patt2d) {
        ...
        try {
            final Scanner scSolfile = new Scanner(solFile);
            int nsols = 0;
            while (scSolfile.hasNextLine()) {
                ...
                if (line.contains("ATOMS IN")) {
                    final int natoms = scSolfile.nextInt();
                    if (natoms > 0) {
            ...
            for (int nsol = 0; nsol < nsols; nsol++) {
                final OrientSolucio os = new OrientSolucio(nsol);
                ...
                while (!endSol) {
                    ...
                    npunts = npunts + 1;
                    final String[] lineS = line.trim().split("\\s+");
                    os.addSolPoint(Integer.parseInt(lineS[0]), Float.parseFloat(lineS[1]), Float.parseFloat(lineS[2]),
                            Integer.parseInt(lineS[3]), Integer.parseInt(lineS[4]), Integer.parseInt(lineS[5]),
                            Float.parseFloat(lineS[6]), Float.parseFloat(lineS[7]));
                }
    ...
    public static File generateTSD(File f, boolean forceExt) {
        if (forceExt)
            f = new File(FileUtils.getFNameNoExt(f).concat(".TSD"));
        try {
    ...
    public static class BatchConvertFileWorker extends SwingWorker<Integer, Integer> {
        ...
        public BatchConvertFileWorker(File[] files) {
            this.flist = files;
        ...
        protected Integer doInBackground() throws Exception {
            ...
            final JTextField txtDistMD = new JTextField(10);
            final JTextField txtWaveL = new JTextField(10);
            txtDistMD.setText("StoD Distance");
            txtWaveL.setText("Wavelength");
            myPanel.add(txtDistMD);
            myPanel.add(txtWaveL);
            final Object[] optionsW = { "Yes, apply these!", "No, take from source images" };
            final int opt = JOptionPane.showOptionDialog(null, myPanel, "Apply Custom Distance and Wavelength?",
                    JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, optionsW, optionsW[1]);
            ...
            final int m = JOptionPane.showOptionDialog(null, "Apply an Excluded Zones file to all images?", "ExZ file",
            ...
            File outdir = null;
            if (!sameDir) {
                outdir = FileUtils.fchooserOpenDir(null, this.flist[0], "Select output directory");
                if (outdir == null) {
                    sameDir = true;
                }
            }
            ...
            for (int i = 0; i < this.flist.length; i++) {
                ...
                File out = new File(
                        FileUtils.getFNameNoExt(this.flist[i]).concat("." + format.toString().toLowerCase()));
                if (!sameDir) {
