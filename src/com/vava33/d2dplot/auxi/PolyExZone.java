...
public class PolyExZone extends Polygon {
    ...
    public PolyExZone(boolean defaultPol4) {
        super();
        if (defaultPol4) {
            this.xpoints = this.defXpoints;
            this.ypoints = this.defYpoints;
            this.npoints = 4;
        } else {
            this.npoints = 0;
            //new empty poligon
    ...
    public boolean contains2(Point2D.Float test) {
        final int[] xs = this.xpoints;
        final int[] ys = this.ypoints;
        int i;
        int j;
    ...
    public boolean equals(Object object) {
        if (object != null && object instanceof PolyExZone) {
            if (this.getBounds().equals(((PolyExZone) object).getBounds()))
                return true;
        }
