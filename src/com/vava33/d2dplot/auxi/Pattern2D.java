...
public class Pattern2D {
    ...
    public Pattern2D(Pattern2D dataIn, boolean copyIntensities){
        ...
        this(dataIn.getDimX(), dataIn.getDimY(), dataIn.getCentrX(),dataIn.getCentrY(),dataIn.getMaxI(),dataIn.getMinI(),dataIn.getScale());
        this.setExpParam(dataIn.getPixSx(), dataIn.getPixSy(), dataIn.getDistMD(), dataIn.getWavel(), dataIn.getTiltDeg(), dataIn.getRotDeg());
        ...
        for (int j = 0; j < this.getDimY(); j++) { // per cada fila (Y)
            for (int i = 0; i < this.getDimX(); i++) { // per cada columna (X)
                if (copyIntensities) {
                	this.setInten(i, j, dataIn.getInten(i, j));
                }else {
                	this.setInten(i, j, 0);
                }
...
//        	this.recalcMaxMinI(); //aixo ja ho fa copyExZonesFromImage
    ...
    public void initIntenArray() {
         ...
         * 
         * per accedir al pixel (x,y)=(3,2) cal anar a la fila 2 columna 3, considerant indexs començant a zero (resto 1 a x,y):
         *          *  
         ...
         *    
         */
        this.pixs = new ArrayList<Pixel>(dimY*dimX);
        ...
        log.debug("pixs size="+pixs.size());
        long end = System.nanoTime();
        float milis = (float) ((end - start) / 1000000d);
        log.debug("init time arraylist= "+(int) milis + " ms)");
    }
    
    public int[][] getIntenAsIntArray(){
    	int[][] inten = new int[dimY][dimX];
        for (int j = 0; j < this.getDimY(); j++) { // per cada fila (Y)
            for (int i = 0; i < this.getDimX(); i++) { // per cada columna (X)
    ...
    public void setInten(int x_col, int y_row, int inten) {
        this.getPixel(x_col, y_row).setIntensity(inten);
    }
    ...
    public boolean isExcluded(int x_col, int y_row) {
        Pixel p = this.getPixel(x_col, y_row);
        if (p.isExcluded())return true;
        if (p.getIntensity()<0)return true;
        return false;
    }
    ...
    public boolean isMouseFreeEzZone(int x, int y) {
    	Iterator<Point> it2 = ExZpaintPixels.iterator();
    ...
    public boolean isInExZoneFullCheck(int x, int y) {
        if (this.isExcludedByThresholds(x,y))return true;
        if (this.isInPolyExZone(x,y))return true;
        if (this.isInArcExZone(x,y))return true;
        if (this.isMouseFreeEzZone(x,y))return true;
        return false;
    }
    ...
    public void calcMeanI(){
        ...
        for (int j = 0; j < this.getDimY(); j++) { // per cada fila (Y)
            for (int i = 0; i < this.getDimX(); i++) { // per cada columna (X)
                if(this.isExcluded(i, j))continue;
                yacum=yacum+this.getInten(i, j);
                npix=npix+1;
                if(this.getInten(i, j)>=getSaturValue()){
                    satur=satur+1;
                    satPixels.add(new Pixel(i,j,getSaturValue()));
                }
            }
        }
        this.nSatur=satur;
        ...
        for (int j = 0; j < this.getDimY(); j++) { // per cada fila (Y)
            for (int i = 0; i < this.getDimX(); i++) { // per cada columna (X)
                if(this.isExcluded(i, j))continue;
                long a = (long) (this.getInten(i, j)-this.meanI);
                long b = a*a;
                double c = b/(float)(npix-1);
                sd = sd + c; 
            }
        }
        ...
        int valorPixelsConsiderarMateixPic = PeakSearch.def_zoneR;
        ...
        for (int i=0;i<nsatur;i++){
            if(idpk[i]==0){
                ...
                for (int j=i;j<nsatur;j++){
                    int arg = (satPixels.get(j).getX()-satPixels.get(i).getX())*(satPixels.get(j).getX()-satPixels.get(i).getX())
                            +(satPixels.get(j).getY()-satPixels.get(i).getY())*(satPixels.get(j).getY()-satPixels.get(i).getY());
    ...
    public float calcMeanI(int minItoConsider){
        ...
        for (int j = 0; j < this.getDimY(); j++) { // per cada fila (Y)
            for (int i = 0; i < this.getDimX(); i++) { // per cada columna (X)
                if(this.isExcluded(i, j))continue;
                int iB4=this.getInten(i, j);
                if(iB4<minItoConsider)continue;
                yacum=yacum+iB4;
                npix=npix+1;
            }
        }
    ...
    public void recalcMaxMinI(){
        ...
        for (int j = 0; j < this.getDimY(); j++) { // per cada fila (Y)
            for (int i = 0; i < this.getDimX(); i++) { // per cada columna (X)
                if(this.isExcluded(i, j))continue;
                if(this.getInten(i, j)>this.getMaxI())this.setMaxI(this.getInten(i, j));
                if(this.getInten(i, j)<this.getMinI())this.setMinI(this.getInten(i, j));
            }
        }
    }
    ...
    public float calcIntSquare(int col_X, int row_Y, int aresta, boolean self){
        ...
        for (int j=row_Y-hA;j<=row_Y+hA;j++){
            for (int i=col_X-hA; i<=col_X+hA;i++){
                ...
                if(this.isExcluded(i, j))continue;
                if((!self)&&(i==col_X)&&(j==row_Y))continue;
                //afegim
        ...
        if(npix>0){
            meanInten = (float)(meanInten+1)/(float)(npix); //recuperem el -1 inicial
        }
        return meanInten;
    }
    
    public float getMax2Tdeg(){
        float f1 = (float) this.calc2T(this.getDimX()-1, this.getDimY()-1, true);
        float f2 = (float) this.calc2T(0,0,true);
        float f3 = (float) this.calc2T(this.getDimX()-1,0,true);
        float f4 = (float) this.calc2T(0,this.getDimY()-1,true);
...
//                    this.setExz_threshold(1);
                    try {
                        fonsPerCercaPk = ImgOps.calcIterAvsq(ImgOps.firstBkgPass(this),NbkgForPksearch,null);
                    ...
                    search8neigbors:
                        for (int jj=j-1;jj<j+2;jj++){
                            for (int ii=i-1;ii<i+2;ii++){
                                if (this.isExcluded(ii, jj))continue;
                    ...
                    if(minpix>1){
                        ...
                        for (int jj=j-nPixAresta_half;jj<=j+nPixAresta_half;jj++){
                            for (int ii=i-nPixAresta_half;ii<=i+nPixAresta_half;ii++){
                                if (!this.isInside(ii,jj))continue;
                                if (this.isExcluded(ii,jj))continue;
