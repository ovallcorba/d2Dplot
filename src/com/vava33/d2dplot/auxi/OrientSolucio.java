...
public class OrientSolucio implements Comparable<Object> {
    ...
    public static int getGrainIdent() {
        return grainIdent;
    }

    public static int getHasFc() {
        return hasFc;
    }
    ...
    public static void setGrainIdent(int grainIdent) {
        OrientSolucio.grainIdent = grainIdent;
    }

    public static void setHasFc(int hasFc) {
        OrientSolucio.hasFc = hasFc;
    }

    public static void setNumSolucions(int numSolucions) {
        OrientSolucio.numSolucions = numSolucions;
    }
    ...
    public void addSolPoint(int refNum, float x, float y, int ih, int ik, int il, float festructura, float foscil) {
        this.sol.add(new PuntSolucio(refNum, x, y, ih, ik, il, festructura, foscil, this.colorSolucio));
    ...
    public void addSolPoint(float x, float y, boolean FromClick) {
        PuntSolucio ps;
        if (isPXY()) {
            final int[] ihkl = this.getNextSOLnumberAsHKL();
            ps = new PuntSolucio(this.getNextSOLnumber(), x, y, ihkl[0], ihkl[1], ihkl[2], 0.1f, 0.0f);
        } else {
            //PREGUNTEM HKL,FSTRUCT AND OSCIL?
            final JPanel myPanel = new JPanel();
            final JTextField txtHKL = new JTextField(10);
            final JTextField txtFstruc = new JTextField(10);
            final JTextField txtOscil = new JTextField(10);
            ...
            try {
                final String shkl[] = txtHKL.getText().split("\\s+");
    ...
    public PuntSolucio getSolPoint(int nr) {
        return this.sol.get(nr);
    }

    public PuntSolucio getLastSolPoint() {
        return this.sol.get(this.sol.size() - 1);
    }
    ...
    public void repintaSolucio() {
        final Iterator<PuntSolucio> itrPunts = this.sol.iterator();
        while (itrPunts.hasNext()) {
            final PuntSolucio ps = itrPunts.next();
    ...
    public void assignaColorSol() {
        if (numSolucions != 0) {
            final int opt = this.getNumSolucio() % 8;
            switch (opt) {
            case 0:
                this.colorSolucio = Color.green;
                break;
            case 1:
                this.colorSolucio = Color.red;
                break;
            case 2:
                this.colorSolucio = Color.cyan;
                break;
            case 3:
                this.colorSolucio = Color.yellow;
                break;
            case 4:
                this.colorSolucio = Color.magenta;
                break;
            case 5:
                this.colorSolucio = Color.orange;
                break;
            case 6:
                this.colorSolucio = Color.pink;
                break;
            case 7:
                this.colorSolucio = Color.blue;
                break;
            }
        } else {
            this.colorSolucio = Color.green;
    ...
    public int getNumReflexions() {
        return this.numReflexions;
    }

    public int getNumSolucio() {
        return this.numSolucio;
    }
    ...
    public ArrayList<Peak> getPeaksPCS() {
        if (this.peaksPCS == null) {
            this.peaksPCS = new ArrayList<Peak>();
        }
        return this.peaksPCS;
    }
    ...
    public void renumberSOLpoints() {
        final Iterator<PuntSolucio> itrps = this.sol.iterator();
        while (itrps.hasNext()) {
            final PuntSolucio ps = itrps.next();
            ps.setSeqNumber(this.sol.indexOf(ps) + 1);
        }
    }
    ...
    public int[] getNextSOLnumberAsHKL() {
        if (this.sol != null) {
            final String shkl = String.format("%03d", this.getNextSOLnumber());
            final char[] chkl = shkl.toCharArray();
            final int[] ihkl = new int[] { Character.getNumericValue(chkl[0]), Character.getNumericValue(chkl[1]),
                    Character.getNumericValue(chkl[2]) };
            return ihkl;
        } else {
    ...
    public PuntSolucio getNearestPS(float px, float py, float maxPixDist) {
        ...
        final Iterator<PuntSolucio> itrS = this.getSol().iterator();
        PuntSolucio nearestPS = null;
        float minModul = maxPixDist;
        while (itrS.hasNext()) {
            final PuntSolucio s = itrS.next();
            final float modul = (float) FastMath
                    .sqrt((s.getCoordX() - px) * (s.getCoordX() - px) + (s.getCoordY() - py) * (s.getCoordY() - py));
            if (modul < minModul) {
                nearestPS = s;
                minModul = modul;
            }
    ...
    public int compareTo(Object arg0) { //EN TOT CAS HAURIEM D'ORDENAR PER VALOR FROT (nrefs coincidents) pero ara al SOL ja surt ordenat
        try {
            final OrientSolucio os = (OrientSolucio) arg0;
    ...
    public String toString() {
        if (this.angR_lon > -1) {
            return String.format("%3d %5d  %.0f alon=%.3f alat=%.3f aspin=%.3f ", this.getGrainNr(),
                    this.getNumReflexions(), this.getNrefsCoincidents(), this.getAngR_lon(), this.getAngS_lat(),
                    this.getAngT_spin());
        } else {
            return String.format("%3d %5d  %.0f", this.getGrainNr(), this.getNumReflexions(),
                    this.getNrefsCoincidents());
        }
