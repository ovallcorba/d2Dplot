...
public class BackgroundPanel extends JPanel
{
    ...
    private void drawScaled(Graphics g)
    {
        ...
        Image reimage = image;
        if ((imgMaxWidth>0)&&(imgMinWidth>=0)){
            int width = Math.max(this.getWidth(), this.imgMinWidth);
            width = Math.min(width, this.imgMaxWidth);
            reimage = image.getScaledInstance(width, -1, java.awt.Image.SCALE_SMOOTH);
        }else{
            reimage = image.getScaledInstance(this.getWidth(), -1, java.awt.Image.SCALE_SMOOTH);
        }
