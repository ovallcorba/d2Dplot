...
public class Ellipse
{
			...
			String title)
	{
		...
		VavaLogger.LOG.info(sb.toString());
	...
	{
		if (FastMath.abs(B) < EPSILON) {
			if (A <= C) {
				// Ellipse is horizontal
				angle = 0d;
				major = FastMath.sqrt(1 / A);
				minor = FastMath.sqrt(1 / C);
			} else {
				// Ellipse is vertical
				angle = FastMath.PI / 2;
				major = FastMath.sqrt(1 / C);
				minor = FastMath.sqrt(1 / A);
			}
		} else {
			// Angle (modulo PI/2)
			double R = (C - A) / B;
			double tg = R - FastMath.sqrt((R * R) + 1);
			angle = FastMath.atan(tg);

			// Axes lengths
			double P = (2 * tg) / (1 + (tg * tg));

			if ((B / P) <= (-B / P)) {
				major = FastMath.sqrt(2 / ((A + C) + (B / P)));
				minor = FastMath.sqrt(2 / ((A + C) - (B / P)));
			} else {
				// Switch
				major = FastMath.sqrt(2 / ((A + C) - (B / P)));
				minor = FastMath.sqrt(2 / ((A + C) + (B / P)));
			...
			///VavaLogger.LOG.info("R=" + R + " tg=" + tg + " P=" + P);
		}
	}
	...
	{
		VavaLogger.LOG.info("-- computeCharacteristics");
			...
			double[] y)
	{
		...
		///VavaLogger.LOG.info("EigenValues");
		 ...
		 */
		if (lambda > 0) {
			distance = FastMath.sqrt(lambda / nbPoints);
		} else {
			distance = 0;
		}
		///VavaLogger.LOG.info("lambda distance=" + distance);
		// Let's try a brutal distance computation
		{
			...
			s /= nbPoints;
			distance = FastMath.sqrt(s);
		}

		///VavaLogger.LOG.info("distance: " + distance);
	}
}